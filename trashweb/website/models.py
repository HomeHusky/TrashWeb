...
class CustomerVouchers(models.Model):
    ...
    GenerateCode = models.CharField(max_length=9, unique=True, editable=False, null=True)
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            ...
            self.GenerateCode = self.Voucher_id.author.Name[:2].upper() + str(random_code)
...
class TrashList(models.Model):
    id = IntegerField(primary_key=True, unique=True, editable=False)
    createat = models.DateTimeField(auto_now_add=True, null=True)
    numoftrash = models.IntegerField('NumTrash', null=True) # Number of types of garbage thrown
    totalscore = models.FloatField('Total Score', max_length=200, null=True)
    description = models.TextField(blank=True)
    iduser = models.ForeignKey(Customer, blank=True, null=True, on_delete=models.CASCADE)
    trash_detail = models.OneToOneField(TrashDetail, null=True, on_delete=models.CASCADE)
